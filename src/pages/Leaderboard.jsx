        ...
        <div className="space-y-6">
          {leaders.map((leader, i) => (
            ...
            >
               ...
               <div className="w-16 h-16 md:w-20 md:h-20 bg-slate-950 border border-slate-800 rounded-2xl overflow-hidden p-1 flex items-center justify-center shrink-0 shadow-2xl transition-all group-hover:border-blue-600">
               ...
               <div className="hidden lg:flex flex-col items-end gap-3 shrink-0">
                  <div className="flex flex-wrap gap-2 justify-end">
                     {leader.offering?.slice(0, 2).map(s => (
                       <span key={s} className="px-3 py-1.5 bg-slate-950 border border-slate-800 text-blue-400 rounded-lg text-[8px] font-black uppercase tracking-widest italic">
                          {s}
                       </span>
                     ))}
                  </div>
                  <div className="w-32 h-1.5 bg-slate-950 rounded-full overflow-hidden border border-slate-800">
                     <div className="h-full bg-blue-600 animate-pulse" style={{ width: `${(leader.rating / 5) * 100}%` }}></div>
                  </div>
               </div>
               
               <div className="absolute top-8 right-8 text-slate-800/10 group-hover:text-blue-500/10 transition-colors pointer-events-none">

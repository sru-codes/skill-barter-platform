    ...
    <div className="max-w-6xl mx-auto py-12 px-6">
      ...
      <div className="mb-16">
        <h1 className="text-5xl md:text-7xl font-black text-white italic tracking-tighter uppercase mb-4 leading-none">
          Exchange <span className="text-indigo-600">Scheduler</span>
        </h1>
          ...
          {filtered.map(book => {
            ...
            return (
              <div key={book.id} className="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-8 md:p-10 flex flex-col group relative overflow-hidden animate-fade-in shadow-2xl">
                <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-600/5 blur-[60px] opacity-0 group-hover:opacity-100 transition-opacity"></div>
                
                <div className="flex items-center justify-between mb-8">
                   <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-slate-950 border border-slate-800 rounded-xl flex items-center justify-center text-indigo-500 shadow-inner">
                         <User size={24} />
                      </div>
                      <div>
                   ...
                   <div className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest italic border
                      ${book.status === 'pending' ? 'bg-yellow-500/10 text-yellow-500 border-yellow-500/20' : 
                        book.status === 'confirmed' ? 'bg-indigo-600/10 text-indigo-500 border-indigo-600/20' : 
                ...
                {book.status === 'pending' && isTarget && (
                  <div className="flex gap-4 mt-auto">
                    <button 
                      ...
                      className="flex-1 btn-primary h-14 text-[10px] font-black uppercase tracking-[0.2em] italic shadow-lg shadow-indigo-600/20"
                    >
                      Authorize Sync
                    </button>
                    <button 
                ...
                {book.status === 'confirmed' && (
                  <div className="flex gap-4 mt-auto">
                     <button 
                        ...
                        className="flex-1 bg-slate-950 border border-slate-800 text-slate-500 hover:text-indigo-500 h-14 text-[10px] font-black uppercase tracking-[0.2em] italic transition-all rounded-xl shadow-lg shadow-black/20"
                     >
                        Finalize Protokol
                     </button>
                     <button className="w-14 h-14 bg-indigo-600 rounded-xl flex items-center justify-center text-white shadow-xl shadow-indigo-600/30">
                        <Rocket size={20} />
                     </button>
                  </div>
                )}

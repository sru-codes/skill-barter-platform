...
export default function Messages() {
  ...
  if (loading) return (
    <div className="h-[60vh] flex items-center justify-center">
       <div className="w-12 h-12 border-4 border-blue-600 border-t-transparent rounded-full animate-spin shadow-[0_0_20px_rgba(79,70,229,0.3)]"></div>
    </div>
  );
    ...
    <div className="max-w-6xl mx-auto py-8 px-4">
      ...
      <div className="relative mb-16 p-12 bg-slate-900 border border-slate-800 rounded-[3.5rem] overflow-hidden shadow-2xl">
        ...
        <div className="relative z-10">
          <h1 className="text-5xl md:text-7xl font-black text-white italic tracking-tighter uppercase mb-4 leading-none strike-blue">
            Barter <span className="text-blue-600">Messages</span>
          </h1>
      ...
      <div className="space-y-6">
        {chats.length === 0 ? (
          <div className="bg-slate-900/50 backdrop-blur-xl border border-dashed border-slate-800 rounded-[4rem] p-32 text-center group hover:border-blue-500/30 transition-all">
            <div className="w-24 h-24 bg-slate-950 border border-slate-800 rounded-3xl flex items-center justify-center mx-auto mb-10 shadow-2xl group-hover:scale-110 transition-transform duration-500">
               <MessageSquare className="text-slate-700 group-hover:text-blue-500 transition-colors" size={40} />
            </div>
            ...
            <button 
              ...
              className="px-12 py-5 bg-blue-600 text-white text-[11px] font-black uppercase tracking-[0.3em] rounded-2xl italic hover:bg-blue-500 transition-all shadow-2xl shadow-blue-600/40"
            >
              Scan For Partners
            </button>
          </div>
          ...
          <div className="grid gap-6">
            {chats.map((chat) => (
              <div 
                ...
                className="group relative bg-slate-900 border border-slate-800 rounded-[2.5rem] p-8 flex flex-col md:flex-row items-center justify-between gap-8 cursor-pointer hover:border-blue-500/50 hover:bg-slate-900/80 transition-all hover:scale-[1.01] shadow-xl overflow-hidden"
              >
                {/* Visual Accent */}
                <div className="absolute top-0 left-0 w-1 h-full bg-blue-600 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                
                <div className="flex items-center gap-8 z-10 w-full md:w-auto">
                  <div className="w-20 h-20 bg-slate-950 border border-slate-800 rounded-2xl flex items-center justify-center text-3xl font-black text-blue-500 shadow-2xl group-hover:border-blue-500/40 transition-all italic">
                    {chat.partnerName?.[0]?.toUpperCase() || "X"}
                  </div>
                  <div>
                    <h3 className="text-2xl font-black text-white uppercase italic tracking-tighter mb-2 group-hover:text-blue-400 transition-colors">
                      {chat.partnerName}
                    </h3>
                ...
                <div className="flex items-center gap-10 z-10 w-full md:w-auto justify-between md:justify-end border-t border-slate-800 pt-6 md:pt-0 md:border-t-0">
                  <div className="hidden lg:block text-right">
                     ...
                     <p className="text-white font-black text-xs uppercase italic tracking-widest text-blue-400">READY TO BARTER</p>
                  </div>
                  <div className="w-14 h-14 rounded-2xl bg-slate-950 border border-slate-800 flex items-center justify-center text-blue-500 group-hover:bg-blue-600 group-hover:text-white transition-all shadow-2xl group-hover:rotate-12">

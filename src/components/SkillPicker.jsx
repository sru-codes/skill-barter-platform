...
export default function SkillPicker({ selected, onToggle, color = "indigo" }) {
  ...
  return (
    <div className="space-y-8">
      <div className="relative group">
        <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-700 group-focus-within:text-indigo-600 transition-colors" size={20} />
        <input 
          type="text" 
          placeholder="Search for skills..."
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          className="w-full bg-slate-950 border border-slate-800 rounded-2xl h-16 pl-14 pr-14 text-white placeholder-slate-800 focus:outline-none focus:border-indigo-600 transition-all font-black uppercase tracking-widest text-xs italic"
        />
        {search && (
      ...
      {!search && (
        <div className="relative flex items-center group/nav">
          <button 
            onClick={() => scroll('left')}
            className="absolute -left-4 z-10 w-10 h-10 bg-slate-900 border border-slate-800 rounded-full flex items-center justify-center text-indigo-500 hover:text-white hover:bg-indigo-600 transition-all opacity-0 group-hover/nav:opacity-100 shadow-xl"
          >
            <ChevronLeft size={20} />
          </button>
          ...
          <button 
            ...
            className="absolute -right-4 z-10 w-10 h-10 bg-slate-900 border border-slate-800 rounded-full flex items-center justify-center text-indigo-500 hover:text-white hover:bg-indigo-600 transition-all opacity-0 group-hover/nav:opacity-100 shadow-xl"
          >
          ...
          <div className="space-y-10">
            {(search ? filteredCategories : [SKILL_CATEGORIES[activeCategory]]).map((cat) => (
              <div key={cat.category} className="animate-fade-in">
                {search && <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-700 mb-5 italic border-l-2 border-indigo-600 pl-4">{cat.category}</p>}
      ...
      <div className="flex justify-between items-center px-2">
        <div className="flex items-center gap-3">
           <div className={`w-2 h-2 rounded-full ${selected.length === 5 ? 'bg-indigo-500 animate-ping' : 'bg-slate-800'}`}></div>
        ...
        {selected.length >= 5 && (
           <p className="text-[9px] text-indigo-500 font-black uppercase tracking-[0.4em] italic animate-pulse">Maximum limit reached</p>
        )}
      </div>
    </div>
  );
